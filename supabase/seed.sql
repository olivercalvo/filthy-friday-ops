...
insert into events (id, date, status, tickets_sold, checked_in, vip_total, vip_cash, vip_card, vip_bottles, merch_units, merch_total, active_venue_id) values
  ...
  ('e1170426-eeee-eeee-eeee-eeeeeeeeeeee', '2026-04-17', 'completed', 478, 451, 6120, 2400, 3720, 24, 87, 1740, '33333333-3333-3333-3333-333333333333'),
  -- Evento activo (default selection)
  ('e2240426-eeee-eeee-eeee-eeeeeeeeeeee', '2026-04-24', 'active',    412, 287, 4850, 2100, 2750, 18, 64, 1280, '22222222-2222-2222-2222-222222222222'),
  -- Evento futuro (draft, números en cero)
  ('e3010526-eeee-eeee-eeee-eeeeeeeeeeee', '2026-05-01', 'draft',       0,   0,    0,    0,    0,  0,  0,    0, '11111111-1111-1111-1111-111111111111');

-- Checklist items por evento
-- Apr 17 (completed) — todo en true
insert into checklist_items (event_id, template_id, venue_id, completed, completed_at, completed_by)
select 'e1170426-eeee-eeee-eeee-eeeeeeeeeeee', t.id, t.venue_id, true,
       timestamp '2026-04-17 09:30:00' + (random() * interval '6 hours'),
       (array['Diana','Rey','Carlos M.','Luis R.','Martina'])[1 + floor(random()*5)::int]
from checklist_templates t;

-- Apr 24 (active) — mix realista
insert into checklist_items (event_id, template_id, venue_id, completed, completed_at, completed_by)
select 'e2240426-eeee-eeee-eeee-eeeeeeeeeeee', t.id, t.venue_id,
       case when random() < 0.55 then true else false end,
       case when random() < 0.55 then timestamp '2026-04-24 10:00:00' + (random() * interval '4 hours') else null end,
       case when random() < 0.55 then (array['Diana','Rey','Carlos M.','Luis R.','Martina'])[1 + floor(random()*5)::int] else null end
from checklist_templates t;

-- May 1 (draft) — todo pendiente
insert into checklist_items (event_id, template_id, venue_id, completed)
select 'e3010526-eeee-eeee-eeee-eeeeeeeeeeee', t.id, t.venue_id, false
from checklist_templates t;
...
insert into alerts (event_id, message, type, venue_id) values
  ('e2240426-eeee-eeee-eeee-eeeeeeeeeeee', 'Check-in superó 250 asistentes en Blue Coconut',           'ok',   '22222222-2222-2222-2222-222222222222'),
  ('e2240426-eeee-eeee-eeee-eeeeeeeeeeee', 'Stock de Corona bajo en Aqua Lounge — enviar refuerzo',    'warn', '33333333-3333-3333-3333-333333333333'),
  ('e2240426-eeee-eeee-eeee-eeeeeeeeeeee', 'Montaje Casa Papaya completado',                            'ok',   '11111111-1111-1111-1111-111111111111');

-- Alertas del evento cerrado (para que la vista "Cuadre" del completed
-- tenga algo que mostrar si el usuario lo selecciona)
insert into alerts (event_id, message, type, venue_id) values
  ('e1170426-eeee-eeee-eeee-eeeeeeeeeeee', 'Evento cerrado — todos los venues finalizaron en horario', 'ok',   null),
  ('e1170426-eeee-eeee-eeee-eeeeeeeeeeee', 'Cuadre VIP final: $6,120 (24 botellas)',                    'info', null);
